...
var REMOTE_MANIFEST_URL = "https://raw.githubusercontent.com/tutivsoft-com/Resources/main/desktop-app-torbert-text-ai.txt";
...
async function generateFileNameFromContent(settings, currentBaseName, text, abortSignal) {
...
async function generateFrontMatterFromContent(settings, currentBaseName, text, abortSignal) {
  const sampledText = buildThreePartSample(text);
...
async function generateTagsFromContent(settings, text, abortSignal) {
...
function applyFrontMatter(text, frontMatter) {
...
function applyTagsFrontMatter(text, tags) {
  ...
  if (/^---\n[\s\S]*?\n---\n?/.test(text)) {
    return text.replace(/^---\n([\s\S]*?)\n---\n?/, (_match, body) => {
      ...
      const withoutExistingTags = body.split("\n").filter((line) => {
...
function sanitizeFileNameStem(value) {
  return value.trim().replace(/^["'`]+|["'`]+$/g, "").replace(/\.md$/i, "").toLowerCase().replace(/&/g, " and ").replace(/[^a-z0-9\s._-]/g, " ").replace(/[\s._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 100).replace(/-+$/g, "");
}
function normalizeTags(value) {
  return value.replace(/^```[\s\S]*?\n/i, "").replace(/```$/i, "").split(/[,\n]+/).map((tag) => sanitizeTag(tag)).filter(Boolean);
}
function sanitizeTag(value) {
  return value.trim().replace(/^["'`#]+|["'`]+$/g, "").toLowerCase().replace(/[^a-z0-9/_-]+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "");
}
...
function normalizeFrontMatter(value) {
  const withoutFences = value.trim().replace(/^```(?:ya?ml)?\s*/i, "").replace(/```$/i, "").trim();
  const body = withoutFences.replace(/^---\s*/, "").replace(/\s*---$/, "").trim();
  return `---
${body}
---`;
}
...
var suggestTitleFromContent = (content, fallback) => {
  var _a;
  const heading = (_a = content.match(/^#\s+(.+)$/m)) == null ? void 0 : _a[1];
  const firstUsefulLine = content.replace(/^---\n[\s\S]*?\n---\n?/, "").split("\n").map((line) => line.replace(/^[-*+]\s+/, "").replace(/^#+\s+/, "").trim()).find((line) => line.length >= 8 && !/^[-*+]?\s*\[[ xX]\]/.test(line));
...
var TorbertTextAiSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("OpenRouter API key").setDesc("Optional. Stored in this vault's local plugin data and sent only to OpenRouter. If blank, Torbert may use its built-in service when available; AI actions still send note text to OpenRouter.").addText((text) => text.setPlaceholder("sk-...").setValue(this.plugin.settings.openAiApiKey).onChange(async (value) => {
...
var BatchPreviewModal = class extends import_obsidian5.Modal {
  constructor(app, title, items, onApply) {
    super(app);
...
var TorbertTextAiPlugin = class extends import_obsidian5.Plugin {
  ...
  async onload() {
    ...
    try {
                ...
                submenu.addItem((categoryItem) => {
                  ...
                  if (hasFileActions) {
                    categoryMenu.addItem((submenuItem) => {
                      submenuItem.setTitle("AI Rename from Content").onClick(() => {
                        if (target instanceof import_obsidian5.TFolder) {
                    ...
                    categoryMenu.addItem((submenuItem) => {
                      submenuItem.setTitle("AI Classify Folder").onClick(() => {
                        if (target instanceof import_obsidian5.TFolder) {
      ...
      new import_obsidian5.Notice("Error: Text Format Helper plugin failed to load. Check developer console.");
    }
  }
  ...
  async renameFileFromContents(file) {
    ...
    try {
      ...
      if (newPath === file.path) {
        new import_obsidian5.Notice(`${file.name} already has the suggested name.`);
        return;
      }
      ...
      if (!processingNotice.wasCancelled()) {
        new import_obsidian5.Notice(`Renamed to ${newPath.split("/").pop() || newPath}.`);
      }
      ...
      new import_obsidian5.Notice(processingNotice.wasCancelled() ? "Operation cancelled." : `Error renaming ${file.name}. Check developer console.`);
    } finally {
      processingNotice.close();
    }
  }
  async renameFilesInFolderFromContents(folder) {
    const files = this.getMarkdownFilesInFolder(folder);
    if (files.length === 0) {
      new import_obsidian5.Notice(`No Markdown files found in ${folder.name}.`);
      return;
    ...
    try {
      ...
      new import_obsidian5.Notice(processingNotice.wasCancelled() ? "Operation cancelled." : `Error planning renames in ${folder.name}.`);
      return;
    } finally {
      processingNotice.close();
    }
    const applyRenames = async () => {
      ...
      try {
        ...
        const failureText = failedCount > 0 ? ` ${failedCount} file(s) failed.` : "";
        new import_obsidian5.Notice(`Renamed ${renamedCount} file(s) in ${folder.name}.${failureText}`);
      } catch (error) {
        this.logger.error("renameFilesInFolderFromContents", `Cancelled or failed applying renames in ${folder.path}`, error);
        new import_obsidian5.Notice(applyNotice.wasCancelled() ? "Operation cancelled." : `Error applying renames in ${folder.name}.`);
      } finally {
        applyNotice.close();
      }
    };
    if (renamePlan.length === 0) {
      new import_obsidian5.Notice(`No file renames suggested in ${folder.name}.`);
      return;
    }
...
Failed files: ${failedCount}` : ""
  ...
  async getAvailableSiblingPath(file, fileName, reservedPaths = /* @__PURE__ */ new Set()) {
    var _a;
    const folderPath = ((_a = file.parent) == null ? void 0 : _a.path) && file.parent.path !== "/" ? `${file.parent.path}/` : "";
    const extension = ".md";
